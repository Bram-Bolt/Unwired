        ...
        if(member === undefined) {
          var x = "goed"
           ...
           hist.forEach(element => {
                   ...
                   var pName = element.name.replace(/"_"/g,"\\_");
                    pNames = pNames + "\n"+ pName
                 })
            ...
            if(check === true){
              mcStatus = "online"
               ...
               let embed = {
                 "title": `Minecraft informatie over ${spelerInfo.name.replace(/"_"/g,"\\_")}` ,
                 "color": "#ffc603",
                 "footer": {
          ...
          .setTitle(`Discord informatie over ${member.user.tag}`)
    			...
    			.setColor("#ffc603")
          .setFooter("Voor minecraft gebruik " + config.prefix + "whois (MC-naam)!", config.mainIcon)
